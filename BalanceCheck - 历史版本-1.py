...
def generate_batch(df):
    ...
    for index, row in df.iterrows():
        ...
        # 查找匹配条件
        match_condition = df[(df['公司代码'] == account_code) & 
                             (df['内部账套编码转换'] == company_code) & 
                             (df['公司代码货币价值'] == -currency_value)]
        ...
        if not match_condition.empty and not match_condition.iloc[0]['匹配批次']:
            ...
            print(f"匹配成功：行{index}和行{match_index}，匹配批次为{match_batch}")
            batch_list.append(match_batch)
            df.at[index, '匹配批次'] = match_batch
            df.at[match_index, '匹配批次'] = match_batch
            serial_number += 1
            success_count += 1
        else:
            print(f"未找到匹配项或匹配批次已存在：行{index}")
            failure_count += 1

    # 打印匹配的总数、成功的总数和失败的总数
    total_matches = len(batch_list)
    print(f"匹配总数：{total_matches}")
    print(f"成功匹配总数：{success_count}")
    print(f"失败匹配总数：{failure_count}")

    return df

# 调用函数生成匹配批次
merged_data_data = generate_batch(merged_data_data)
...
# 创建一个新的Excel writer
with pd.ExcelWriter(os.path.join(current_directory, '统计.xlsx')) as writer:
